...
/** 月数组 */
@property (nonatomic, strong) NSArray *monthArrM;
@end
...
- (void)initPickView
{
    ...
    NSString *currentDate = [dateFormatter stringFromDate:mydate];
    
    NSInteger currentMonth = [currentDate substringWithRange:NSMakeRange(5, 2)].integerValue;
    NSMutableArray *arrM = [NSMutableArray array];
    NSInteger tempMonth = currentMonth;
    BOOL isOtherYear = NO;
    [arrM addObject:[NSNumber numberWithInteger:currentMonth]];
    for (NSInteger i = 0; i < 2; i++) {
        ...
        [arrM addObject:[NSNumber numberWithInteger:tempMonth]];
    }
    _monthArrM = [arrM reverseObjectEnumerator].allObjects;
    
    _minShowYear = [currentDate substringToIndex:4].integerValue;//最小年份
    if (isOtherYear) {
        _minShowYear -= 1;
    }
...
-(void)setDefaultTSelectYear:(NSInteger)defaultSelectYear defaultSelectMonth:(NSInteger)defaultSelectMonth defaultSelectDay:(NSInteger)defaultSelectDay{
    ...
    [_monthArrM enumerateObjectsUsingBlock:^(NSNumber * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if (obj.integerValue == _defaultMonth) {
...
- (NSInteger)pickerView:(UIPickerView *)pickerView numberOfRowsInComponent:(NSInteger)component
{
    if (component == 0) {
        ...
        NSInteger yearSelected = [pickerView selectedRowInComponent:0] + self.minShowYear;
        NSInteger monthSelected = [_monthArrM[[pickerView selectedRowInComponent:1]] integerValue];
...
//            text =  [NSString stringWithFormat:@"%zd月", row + 1];
            text =  [NSString stringWithFormat:@"%zd月", [_monthArrM[row] integerValue]];
        }
...
- (void)refreshPickViewData
{
    ...
    self.selectMonth = [_monthArrM[[self.pickerView selectedRowInComponent:1]] integerValue];
    if (_isShowDay==YES) {
